...
def check_winner(board):
    ...
    for r in range(rows):
        for c in range(cols - 3):
            if board[r][c] == board[r][c + 1] == board[r][c + 2] == board[r][c + 3] and board[r][c] is not None:
                return board[r][c]
    
    # Vertical win
    for r in range(rows - 3):
        for c in range(cols):
            if board[r][c] == board[r + 1][c] == board[r + 2][c] == board[r + 3][c] and board[r][c] is not None:
                return board[r][c]
            
    # Diagonal-down win
    for r in range(rows - 3):
        for c in range(cols - 3):
            if board[r][c] == board[r + 1][c + 1] == board[r + 2][c + 2] == board[r + 3][c + 3] and board[r][c] is not None:
                return board[r][c]
